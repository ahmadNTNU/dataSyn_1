...
def main():
    ...
    for i in range(10):
        ...
        plt.imshow(model1.w[:-1, i].reshape(28, 28), cmap='gray')
        plt.title(f'Digit {i}')
        plt.axis('off')
        plt.subplot(2, 10, i+11)
        plt.imshow(model2.w[:-1, i].reshape(28, 28), cmap='gray')
        plt.title(f'Digit {i}')
        plt.axis('off')
    plt.suptitle('Weights Visualization for λ = 0.0/1', fontsize=16)
    plt.tight_layout()
    plt.savefig("task4b_l2_regularization.png")
    plt.show() '''
    plt.imsave("task4b_softmax_weight.png", model1.w, cmap="gray")
    plt.imsave("task4b_softmax_weight.png", model2.w, cmap="gray")
    ...
    for i in l2_lambdas:
        ...
        utils.plot_loss(val_history_reg["accuracy"], f"λ={i}")

    plt.xlabel("Number of Training Steps")
    plt.ylabel("Validation Accuracy")
    plt.title("Validation Accuracy for Different Values of λ")
    plt.legend()
    plt.grid(True)
    ...
    for i in l2_lambdas:
        ...
        train_history_reg, val_history_reg = trainer.train(num_epochs)
        plt.plot(i, np.linalg.norm(model.w), marker='o')          
