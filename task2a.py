...
def pre_process_images(X: np.ndarray):
    ...
    #adding a column of 1 as the bias 
    X = np.column_stack((X, np.ones_like(X[:, 0])))
    X_min = X.min()
    X_max = X.max()
    X_range = X_max - X_min 
    # transforming X to [0,1]
    X = (X-X_min)/float(X_range)
    #transforming X to the range [-1,1]
    X = -1+(X*2)
...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray) -> float:
    ...
    """
    # TODO implement this function (Task 2a)
    #C(w)=(1/N)*sum(Cn) where Cn(w) = −(ynln(ˆyn) + (1 − yn) ln(1 − yˆn)).
...
class BinaryModel:
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO implement this function (Task 2a)
        # we have that f(x)= 1/(1+e^(-w^t*x)) and W^t*x = sum(wi*xi)
        Z = np.dot(X, self.w)
        y = 1/(1+np.exp(-Z))
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        # TODO implement this function (Task 2a)
        # gradient = -(yn-y^n)xi
        gradient = -np.dot(X.T, (targets - outputs))/len(X)
        self.grad = gradient 
